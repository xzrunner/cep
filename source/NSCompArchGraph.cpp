...
void NSCompArchGraph::LoadFromBin(const ur::Device& dev, const std::string& dir, bs::ImportStream& is)
{
}
...
void NSCompArchGraph::LoadFromJson(const ur::Device& dev, mm::LinearAllocator& alloc, const std::string& dir, const rapidjson::Value& val)
{
    auto relative = val["filepath"].GetString();
    m_filepath = boost::filesystem::absolute(relative, dir).string();
}

void NSCompArchGraph::StoreToMem(const ur::Device& dev, CompArchGraph& comp) const
{
